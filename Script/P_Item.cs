...
{    
   public GameObject XX_item;
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.gameObject.tag == "Coin") // 코인습득
        {
        ...
        if (collision.gameObject.tag == "HP") // 체력회복
        {
            ui_manager.HP_plus();
        ...
        if (collision.gameObject.tag == "STE") // 스태미나회복
        {
            ui_manager.STE_plus();
            item_manager.itemList.Remove(item_collision);
            Destroy(item_collision);
        ...
        if (collision.gameObject.tag == "garbage") // 쓰레기 치우기
        {
